...
跑出来。两种情况下都没有内置 ArbiterOS 的任何代码。署名、许可证和所做的改动都在
...
而在于有一个数字会被读成它的反面，而且那恰好是一个红队用例集的作者最先会看的数字。
`intercepted` 是 1：105 条用例里，内核真正改写了的响应只有一个，
`openclaw_p9_process_poll_loop`，被 `RateLimitPolicy` 按 per-tool 额度拦下。`flagged` 是 66，
而包里那句话把它说成"命中了策略的用例数"。实测下来，它不是。66 里有 65 是 `UnaryGatePolicy`
把工具调用的参数按不同的键顺序重新序列化了一遍，没有任何规则命中，也没有摘掉任何东西——
这一条是在全部 105 条上核过的，不是抽样：把每个 `arguments` 字符串解析回 JSON 之后，策略链拿到的
响应与它交回的响应每一条都相等。**这两个数字都不是这个用例集的检出率，也都不该被这样读。**

这次运行真正说明了什么，测量过程在
[`scripts/arbiteros-runner/README.md`](scripts/arbiteros-runner/README.md) 里，下面还有逐条策略的
表。全部十五条已注册策略在每条用例上都会跑；`policy_registry.json` 里的 `enabled` 决定的只是
命中之后会怎样，而随包发布的默认配置里十五条中有四条是执行模式。把每条策略在这层门禁之前的判定
记下来：105 条用例里有 39 条产生了拒绝——用例集标为 unsafe 的 45 条里占 21 条，标为 safe 的 60 条里
占 18 条——而其中 38 条的这类策略全部是"只观察"的，于是拒绝被算了出来，然后被丢掉。这 38 条不在
这个包里：内核把它们放在 `inactivate_error_type` 里交回来，而运行脚本还没有记录这个字段。所以这个
包今天支持的说法是"随包配置**执行**了什么"，而不是"这个用例集**检出**了什么"，而这两者相距很远。
